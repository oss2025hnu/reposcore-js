...
const CSV_HEADER = 'name,feat/bug PR count,feat/bug PR score,doc PR count,doc PR score,typo PR count,typo PR score,feat/bug issue count,feat/bug issue score,doc issue count,doc issue score,total,issue_ratio,doc_ratio,typo_ratio\n';
...
class CsvGenerator {
    ...
    async generateCombinedCsv(repoName, repoScores, repoActivities, outputDir) {
        ...
        for (const [participant, activities] of repoActivities.entries()) {
            ...
            // Calculate featRatio and issueRatio as percentages
            ...
            csvContent += `${participant},${p_fb_count},${p_fb_score},${p_d_count},${p_d_score},${p_t_count},${p_t_score},${i_fb_count},${i_fb_score},${i_d_count},${i_d_score},${total},${docRatio}%,${typoRatio}%,${issueRatio}%\n`;
