...
class RepoAnalyzer {
    ...
    async collectPRsAndIssues() {
        const analyzer = new ContributionAnalyzer(EXCLUDE_USERS);
    
        for (const repoPath of this.repoPaths) {
            const [owner, repo] = repoPath.split('/');
            const stats = new ContributionStats();
    
            const items = await this._fetchAllItems(owner, repo);
    
            items.forEach(item => {
                const contribution = item.pull_request
                    ? analyzer.analyzePullRequest(item)
                    : analyzer.analyzeIssue(item);
            ...
            this.participants.set(repo, stats.generateSummary());
        }
    }

    async _fetchAllItems(owner, repo) {
        let page = 1;
        let allItems = [];
